 ...
 * between ursad, and ursa-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<ursaaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by URSA Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bigursula/ursa>";
    const std::string URL_WEBSITE = "<https://bigursula.com>";
...
/** Sanity checks
 *  Ensure that Ursa Core is running in a usable environment with all
...
{
    // Make sure only a single Ursa Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Ursa Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Ursa Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule ursa-specific tasks
