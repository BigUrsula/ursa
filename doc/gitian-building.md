...
*Setup instructions for a Gitian build of Ursa Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the URSA
...
- [Building Ursa Core](#building-ursa-core)
...
Clone the git repositories for Ursa Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bigursula/ursa
...
in the Ursa Core repository under 'Fetch and create inputs' to install sources which require
...
To build Ursa Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Ursa Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ursa/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bigursula/ursa
...
For example:
```bash
URL=https://github.com/crowning-/ursa.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ursa=${COMMIT} --url ursa=${URL} ../ursa/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ursa=${COMMIT} --url ursa=${URL} ../ursa/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ursa=${COMMIT} --url ursa=${URL} ../ursa/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ursa git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ursa/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ursa
SIGPATH=/some/root/path/ursa-detached-sigs

./bin/gbuild --url ursa=${BTCPATH},signature=${SIGPATH} ../ursa/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ursa-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ursa-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ursa-osx-build.assert
```
